...
Monster Offer Windows 桌面版需要降低线上会议、桌面共享和常见录屏工具捕获本应用窗口内容的风险。保护对象包括悬浮控制条、实时转写、AI 回答和设置窗口。

本方案只保护 Monster Offer 自己创建的窗口和本地数据，不控制 Zoom、Teams、OBS 或其他第三方进程，也不实现进程隐藏、反调试、API Hook、DLL 注入、窗口伪装或绕过第三方安全检测。
...
```text
Electron 主进程
  ├─ WindowPrivacyManager：管理所有 Monster Offer 顶层窗口
...
主进程必须验证 IPC sender 属于已创建的 Monster Offer 窗口，并拒绝未知 channel、任意脚本参数和跨窗口控制。渲染进程保持 `nodeIntegration: false`、`contextIsolation: true` 和沙箱配置，不直接访问文件系统、环境变量或 API 密钥。
...
2. 上述工具共享整个桌面和多显示器桌面。
3. Monster Offer 窗口在本机仍可操作，捕获结果中按工具能力被排除或显示黑色区域。
